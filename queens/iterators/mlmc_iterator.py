...
class MLMCIterator(Iterator):
    ...
    def __init__(
        ...
        """
        if isinstance(drivers, list) and isinstance(num_samples, list) and len(num_samples) == len(drivers):
    ...
    def core_run(self):
        ...
        pivot = self.num_samples[0] # index to select the right samples out of sample list for each level, gets updated throughout for-loop
        ...
        for i in range(1, len(self.models)):
            samples = self.samples[pivot:pivot+self.num_samples[i]] # samples index by number of samples on each level and extracted from samples array

            mean += (self.models[i].evaluate(samples)['result'] - self.models[i-1].evaluate(samples)['result']).mean() #Difference between levels is added to the mean
