...
- `input.pickle`: The input parameters that QUEENS injects into the input template.
- `jobscript.sh`: Contains the commands required to execute your code.
- `metadata.yaml`: Provides metadata from the simulation run, detailing timings, successful execution, and additional information.
- `output.pickle`: The outputs that QUEENS extracts from the output files using a data processor.
